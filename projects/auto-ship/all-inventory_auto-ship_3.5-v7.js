...
await page.waitForFunction(() => document.readyState === 'complete');
 ...
 while (orderExists !== "no orders to process") {
    ...
    if (orderExists !== "no orders to process") {
        ...
        console.log('Re-filtering grid to clear shipped orders...');
        await page.click('[onclick="PackingSlipLineItemList.Search()"]');
        await waitForJsonResponse(page, 'https://admin.marcomcentral.app.pti.com/LineItem/PackingSlipLineItemListDataRequested?_search=true', `PortalName=${portalName}`, 10000);
