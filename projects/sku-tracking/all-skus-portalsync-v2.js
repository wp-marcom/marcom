...
let fullSKUdescription = r[4];
    ...
    // Try the OLD format first (with HTML tags)
    let locationText = /Location:<\/b><\/span>(.*)/.exec(fullSKUdescription);
    
    // If old format didn't match, try the NEW format (with pipe)
    if (!locationText) {
      //locationText = /\|\s*Location:\s*(.*)/.exec(fullSKUdescription);
      locationText = /Location:\s*(.*)/.exec(fullSKUdescription);
      console.log(`New SKU description found`);
    }
    
    let locationInfo = locationText ? locationText[1].trim() : ''; // Extract and trim

        //let match = /Location:<\/b><\/span>(.*)/.exec(r[4]);
        //let locationText = match ? match[1].trim() : ''; // Extract the matched text and trim whitespace
        return [r[2], r[3], locationInfo, r[11], r[12], r[15], r[16], r[18]];
      });
      data.shift();
      data.shift();
      data.shift();
